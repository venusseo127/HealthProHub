...
import { getPatientById, updatePatient, getTreatmentLogs, getAdmissions, getBillings, getDietPlans } from "@/lib/firebase";
  ...
  useEffect(() => {
    ...
    const fetchPatientData = async () => {
      ...
      try {
        ...
        // Fetch related data
        const treatmentResult = await getTreatmentLogs(undefined, params.id);
        setTreatments(treatmentResult.treatments || []);
        
        const admissionResult = await getAdmissions(params.id);
        setAdmissions(admissionResult.admissions || []);
        
        const billingResult = await getBillings(params.id);
        setBillings(billingResult.billings || []);
        
        if (user?.role === "nurse") {
          const dietResult = await getDietPlans(params.id);
